...
The action will be carried out by this function.  This allows you to
control over whether and when the "action" will be performed.
...
`url_action('lucky_color')`, the interview will load as usual, but
when **docassemble** processes the `initial` code block, it will run
the function `process_action(current_info)`, which will cause
**docassemble** to look for a `question` or `code` block that defines
`lucky_color`.  Since there is a question in the interview that
...
You can pass as many named parameters as you like in a `url_action()`
function.
...
code: |
  ...
  user_salary = current_info['arguments']['money']
...
When the user clicks one of the links, the interview will load as usual (much
as if the user refreshed the web browser page).  The only difference
is that **docassemble** will see the additional information stored in
the URL and will make that information available to the interview by
storing it in the `current_info` [dictionary].


The `url_action()` function can also execute code and pass along
additional information to the code.  For example,
`url_action('set_number_event', increment=1)` causes **docassemble**
to look for a `question` or `code` block that promises to define
`set_number_event`.  It will find the block marked with `event:
set_number_event` and will run the code.  The code is able to retrieve
the `increment` by accessing the built-in `current_info` [dictionary].
