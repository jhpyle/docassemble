...
`url_action()` returns a URL that will perform an action within
**docassemble**, such as running some code or asking a question.
Typically you will plug this URL into a [Markdown] link.

In order for this action to be performed, you need to create a block
of `initial` code containing `process_action(current_info)`.  You can
specify when and if you want the code to be run.
...
`url_action('lucky_color')`, **docassemble** effectively runs
`force_ask('lucky_color')`.  This happens when **docassemble** runs
the code `process_action(current_info)`.
