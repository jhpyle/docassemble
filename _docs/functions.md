...
[`interview_url_action()`] (after calling [`update_info()`] to enable
this function), and cause actions to run with [`process_action()`].
...
3. Using [`initial`] code, we run [`update_info()`] to provide the
   [`interview_url_action()`] and [`url_action()`] functions with the
   information they need to operate.
4. The `query_fruit` [`event`] code will be run as an [action] when
...
* [`multi_user`] must be set to `True`.  This disables server-side
  ...
  a URL.
* [`initial`] code runs [`update_info()`] (which is needed by
...
will behave differently depending on background information about the
...
3. Calls `update_info(current_info, user=user, role=role)`.  (The
   [`current_info`] dictionary is already defined by **docassemble**.)
...
code: |
  if current_info['user']['is_authenticated'] and \
    ...
    user = client
    role = 'client'
  update_info(current_info, user=user, role=role)
---
{% endhighlight %}
...
initial: true
code: |
  update_info(current_info, user=user, role='interviewee', 
