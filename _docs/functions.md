...
# Functions for interacting with the interview using URLs
...
a `question` or `code` block that defines `lucky_color`.  (It calls
[`force_ask()`], explained above.)  Since there is a question in the
interview that defines `lucky_color`, that question will be asked.

When the user clicks on the link generated by
`url_action('set_number_event', increment=1)`, the
`process_action()` function will look for a `question` or
...
placing the `process_action()` statement in a particular
...
such as a cron job.  For information on how to do this, see [`interview_url_action()`].
...
different units.  The function takes two arguments
(`date_difference(a, b)`).  If date `a` comes after date `b`, the
values will be positive, but if date `b` comes after date `a`, the
values will be negative.

For example, if you set `a = date_difference('1/3/2015', '1/1/2015')`,
then:

* `a.weeks` returns ``.
* `a.days` returns `2.0`.
* `a.hours` returns `48.0`.
* `a.minutes` returns `2880.0`.
* `a.seconds` returns `172800`.
* `a.delta` returns a [`datetime.timedelta`] object representing the
