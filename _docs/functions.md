...
that uses a [`docx template file`], where you are passing values to
the .docx file using the `code` or `field code` methods.
...
displayed in the .docx file as an image.  Or, if the value contains
