            ...
            if hasattr(status.question.fields[0], 'saveas'):
                if hasattr(status.question.fields[0], 'has_code') and status.question.fields[0].has_code:
                    for pair in status.selectcompute[status.question.fields[0].saveas]:
                        output += '<div class="radio"><label for="' + status.question.fields[0].saveas + '"> ' + pair[1] + '</label><input class="pretty" name="' + status.question.fields[0].saveas + '" id="' + status.question.fields[0].saveas + '" type="radio" value="' + pair[0] + '"></div>'
                else:
                ...
                for choice in status.question.fields[0].choices:
                    ...
                    for key in choice:
                        if key == 'image':
                            continue
                        output += '<div class="radio"><label for="multiple_choice">' + the_icon + key + '</label><input class="pretty" name="multiple_choice" id="multiple_choice" type="radio" value="' + str(indexno) + '"></div>'
                    indexno += 1
...
def input_for(status, field):
    ...
    if field.datatype == 'selectcompute':
        ...
        output += '<input class="pretty" type="checkbox" value="True" name="' + field.saveas + '" id="' + field.saveas + '"'
        if defaultvalue:
            output += ' checked'
        output += '> ' + field.label
