...
class DummyPackage:
    """Object that mimics a Package, for use when manually uninstalling a package"""
    def __init__(self, name):
        self.name = name
        self.type = 'pip'
        self.limitation = None

INSTALLED_DISTRIBUTION_CACHE: List[Object] = []
...
def remove_inactive_hosts(start_time=None):
    ...
    if USING_SUPERVISOR:
        ...
        with session_scope() as session:
            for host in session.execute(select(Supervisors)).scalars():
            ...
            for id_to_delete in to_delete:
                session.execute(delete(Supervisors).filter_by(id=id_to_delete))
...
def clear_invalid_package(start_time=None):
    ...
    package_a = aliased(Package, name='a')
    package_b = aliased(Package, name='b')
    with session_scope() as session:
        result = session.execute(
            select(package_a.id)
            .join(package_b, package_a.name.ilike(package_b.name))
            .filter(package_b.active == True)  # noqa: E712 # pylint: disable=singleton-comparison
            .filter(package_b.id > package_a.id)
            .group_by(package_a.id)).all()
        for item in result:
            logmessage("clear_invalid_package: deleting duplicate package id " + str(item.id))
            session.execute(delete(Package).filter_by(id=item.id))
...
def clear_invalid_package_auth(start_time=None):
    ...
    with session_scope() as session:
        session.execute(delete(PackageAuth).filter_by(package_id=None))
...
def check_for_updates(start_time=None, invalidate_cache=True, full=True):
    ...
    with get_session() as session:
        if full:
            ...
            should_commit = False
            for package_name in ('psycopg2', 'pdfminer', 'pdfminer3k', 'py-bcrypt', 'pycrypto', 'constraint', 'distutils2', 'azure-storage', 'Flask-User', 'Marisol', 'sklearn', 'backports.zoneinfo', 'Flask-Babel', 'typing-extensions', 'Babel', 'pyasn1-modules', 'Pillow', 'importlib-metadata', 'importlib-resources', 'prompt-toolkit', 'readme-renderer', 'parse-type', 'et-xmlfile', 'docassemble'):
                result = session.execute(delete(Package).filter_by(name=package_name))
        ...
        for package in session.execute(select(Package).filter_by(active=True)).scalars():
        ...
        for package in session.execute(select(Package).filter_by(active=False)).scalars():
        ...
        for auth in session.execute(select(PackageAuth).filter_by(authtype='owner')).scalars():
        ...
        for package in to_uninstall:
            ...
            if returnval == 0:
                with session_scope() as session:
                    session.execute(delete(Install).filter_by(hostname=hostname, package_id=package.id))
...
def fix_names():
    ...
    with session_scope() as session:
        for package in session.execute(select(Package).filter_by(active=True).with_for_update()).scalars():
...
def install_package(package, start_time=None):
    ...
    if package.type == 'zip' and package.upload is None:
        return 0, ''
    logfilecontents = ''
    with tempfile.NamedTemporaryFile() as pip_log:
...
def invalidate_installed_distributions_cache():
    INSTALLED_DISTRIBUTION_CACHE.clear()


def get_installed_distributions(start_time=None):
    if len(INSTALLED_DISTRIBUTION_CACHE) > 0:
    ...
    INSTALLED_DISTRIBUTION_CACHE.extend(results)
