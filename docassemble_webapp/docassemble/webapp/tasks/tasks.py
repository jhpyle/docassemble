...
from docassemble.base.functions import set_uid
from docassemble.base.hooks import retrieve_email
from docassemble.base.interview_cache import get_interview
from docassemble.base.language.control import get_language, set_language
...
from docassemble.base.util import (
    ...
    ocr_finalize as do_ocr_finalize,
    make_png_for_pdf as do_make_png_for_pdf,
    google_ocr_file,
)
...
from docassemble.webapp.develop.oauth import RedisCredStorage
...
from docassemble.webapp.main.helpers import trigger_update
from docassemble.webapp.tasks.common import (
    celery_app,
    process_error,
    error_object,
    ReturnValue,
)
from docassemble.webapp.update import check_for_updates
from docassemble.webapp.users.helpers import get_user_object, update_last_login
...
@celery_app.task(name='tasks.sync_with_google_drive')
def sync_with_google_drive(user_id):
    ...
    storage = RedisCredStorage(r, user_id, oauth_app='googledrive')
    ...
    try:
        with flaskapp.app_context():
            ...
            for section in ['static', 'templates', 'questions', 'modules', 'sources', 'packages']:
                ...
                while True:
                    ...
                    response = service.files().list(**param).execute()  # pylint: disable=no-member
                ...
                if len(subdirs) == 0:
                    ...
                    new_dir = service.files().create(body=file_metadata, fields='id').execute()  # pylint: disable=no-member
                    ...
                    if f not in local_files[section] or gd_modtimes[section][f] - local_modtimes[section][f] > 3:
                        ...
                        if f in gd_zero[section]:
                            ...
                            with open(the_path, 'wb') as fh:
                                response = service.files().get_media(fileId=gd_ids[section][f])  # pylint: disable=no-member
                ...
                for f in local_files[section]:
                    ...
                    if f not in gd_deleted[section]:
                        ...
                        if f not in gd_files[section]:
                            ...
                            extension, mimetype = get_ext_and_mimetype(the_path)  # pylint: disable=unused-variable
                            ...
                            if dir_part != '':
                                if dir_part not in gd_dirlist[section]:
                                    ...
                                    new_file = service.files().create(body=file_metadata, fields='id').execute()  # pylint: disable=no-member
                            ...
                            the_new_file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()  # pylint: disable=no-member
                            the_new_file.get('id')
                            ...
                            media = apiclient.http.MediaFileUpload(the_path, mimetype=mimetype)
                            updated_file = service.files().update(fileId=gd_ids[section][f], body=file_metadata, media_body=media, fields='modifiedTime').execute()  # pylint: disable=no-member
                ...
                for f in os.listdir(area.directory):
                    ...
                    if abs(local_modtimes[section][f] - gd_modtimes[section][f]) > 3:
                        ...
                        file_metadata = {'modifiedTime': the_modtime}
                        updated_file = service.files().update(fileId=gd_ids[section][f], body=file_metadata, fields='modifiedTime').execute()  # pylint: disable=no-member
                        gd_modtimes[section][f] = epoch_from_iso(updated_file['modifiedTime'])
        ...
        return ReturnValue(ok=False, error="Error syncing with Google Drive: " + noquote(str(e)), restart=False)
...
def try_request(*pargs, **kwargs):
    ...
    tries = 1
    while tries < 5:
        req, content = http.request(*args, **kwargs)
        if int(req['status']) != 504:
            break
...
def sync_with_onedrive(user_id):
    ...
    try:
        with flaskapp.app_context():
            ...
            # req, content = try_request(http, "https://graph.microsoft.com/v1.0/drive/special/approot")
            ...
            the_folder = the_folder.decode()
            req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(the_folder), "GET")
            if int(req['status']) != 200:
                trashed = True
            else:
            ...
            subdirs = {}
            subdir_count = {}
            req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(the_folder) + "/children?$select=id,name,deleted,folder", "GET")
            while True:
                if int(req['status']) != 200:
                ...
                if "@odata.nextLink" not in info:
                    break
                req, content = try_request(http, info["@odata.nextLink"], "GET")
            for section in ['static', 'templates', 'questions', 'modules', 'sources', 'packages']:
                ...
                if subdir_count[section] == 0:
                    ...
                    req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(subdirs[section]) + "/children?$select=id,name,deleted,fileSystemInfo,folder,size", "GET")
                    logmessage("sync_with_onedrive: processing " + section + ", which is " + str(subdirs[section]))
                    while True:
                        if int(req['status']) != 200:
                            return ReturnValue(ok=False, error="error accessing OneDrive subfolder " + section + " " + str(req['status']) + ": " + content.decode() + " looking for " + str(subdirs[section]), restart=False)
                        info = json.loads(content.decode())
                    ...
                    for subdir_name, subdir_id in od_dirlist[section].items():
                        req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(subdir_id) + "/children?$select=id,name,deleted,fileSystemInfo,folder,size", "GET")
                        logmessage("sync_with_onedrive: processing " + section + " subdir " + subdir_name + ", which is " + str(subdir_id))
                        while True:
                            if int(req['status']) != 200:
                                return ReturnValue(ok=False, error="error accessing OneDrive subfolder " + section + " subdir " + subdir_name + " " + str(req['status']) + ": " + content.decode() + " looking for " + str(subdir_id), restart=False)
                            info = json.loads(content.decode())
                            for the_file in info['value']:
                                if 'folder' in the_file:
                                    continue
                ...
                for f in od_files[section]:
                    ...
                    if f not in local_files[section] or od_modtimes[section][f] - local_modtimes[section][f] > 3:
                        ...
                        if f in od_zero[section]:
                            ...
                            req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(od_ids[section][f]) + "/content", "GET")
                            with open(the_path, 'wb') as fh:
                                fh.write(content)
                            os.utime(the_path, (od_modtimes[section][f], od_modtimes[section][f]))
                for f in local_files[section]:
                        ...
                        if f not in od_files[section]:
                            ...
                            if dir_name != '':
                                if dir_name not in od_dirlist[section]:
                                    ...
                                    req, content = http.request("https://graph.microsoft.com/v1.0/me/drive/items/" + str(subdirs[section]) + "/children", "POST", headers=headers, body=json.dumps(dirdata))
                                    if int(req['status']) != 201:
                                        raise DAError("sync_with_onedrive: could not create subfolder " + dir_name + ' in ' + str(subdirs[section]) + '.  ' + content.decode() + ' status: ' + str(req['status']))
                                    new_item = json.loads(content.decode())
                ...
                for f in os.listdir(area.directory):
                    ...
                    if abs(local_modtimes[section][f] - od_modtimes[section][f]) > 3:
                        ...
                        headers = {'Content-Type': 'application/json'}
                        req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(od_ids[section][f]), "PATCH", headers=headers, body=json.dumps({'fileSystemInfo': {"createdDateTime": iso_from_epoch(od_createtimes[section][f]), "lastModifiedDateTime": the_modtime}}, sort_keys=True))
                        if int(req['status']) != 200:
                            return ReturnValue(ok=False, error="error updating OneDrive file in subfolder " + section + " " + str(req['status']) + ": " + content.decode(), restart=False)
                        od_modtimes[section][f] = local_modtimes[section][f]
            for key in r.keys('da:interviewsource:docassemble.playground' + str(user_id) + ':*'):
                r.incr(key)
            if commentary != '':
...
def onedrive_upload(http, folder_id, folder_name, data, the_path, new_item_id=None):
    ...
    if total_bytes == 0:
        req, content = try_request(http, 'https://graph.microsoft.com/v1.0/me/drive/items/' + quote(folder_id) + ':/' + quote(data['name']) + ':/content', 'PUT', headers={'Content-Type': 'text/plain'}, body=bytes())
        if int(req['status']) not in (200, 201):
            logmessage("Error0")
            logmessage(str(req['status']))
            logmessage(content.decode())
            return ReturnValue(ok=False, error="error uploading zero-byte file to OneDrive subfolder " + folder_id + " " + str(req['status']) + ": " + content.decode(), restart=False)
        ...
        req, content = try_request(http, the_url, 'POST', headers=headers, body=json.dumps(body_data, sort_keys=True))
        if int(req['status']) != 200:
            return ReturnValue(ok=False, error="error uploading to OneDrive subfolder " + folder_id + " " + str(req['status']) + ": " + content.decode() + " and url was " + the_url + " and folder name was " + folder_name + " and path was " + the_path + " and data was " + json.dumps(body_data, sort_keys=True) + " and is_new is " + repr(is_new), restart=False)
        ...
        with open(the_path, 'rb') as fh:
            while start_byte < total_bytes:
                ...
                req, content = try_request(http, upload_url, 'PUT', headers=custom_headers, body=bytes(fh.read(num_bytes)))
                ...
                if start_byte == total_bytes:
                    ...
                    if int(req['status']) not in (200, 201):
                        ...
                        logmessage(str(req['status']))
                        logmessage(content.decode())
                        return ReturnValue(ok=False, error="error uploading file to OneDrive subfolder " + folder_id + " " + str(req['status']) + ": " + content.decode(), restart=False)
                    if new_item_id is None:
                        new_item_id = json.loads(content.decode())['id']
                else:
                    if int(req['status']) != 202:
                        logmessage("Error2")
                        logmessage(str(req['status']))
                        logmessage(content.decode())
                        return ReturnValue(ok=False, error="error during upload of file to OneDrive subfolder " + folder_id + " " + str(req['status']) + ": " + content.decode(), restart=False)
                    logmessage("Got 202")
    ...
    req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(new_item_id), "PATCH", headers=headers, body=json.dumps(item_data, sort_keys=True))
    logmessage("PATCH request sent")
    if int(req['status']) != 200:
        return ReturnValue(ok=False, error="error during updating of uploaded file to OneDrive subfolder " + folder_id + " " + str(req['status']) + ": " + content.decode(), restart=False)
    # tries = 1
    # start_time = time.time()
    # while tries < 3:
    ...
    #     req, content = try_request(http, "https://graph.microsoft.com/v1.0/me/drive/items/" + quote(new_item_id), "GET")
    #     if int(req['status']) != 200:
    #         return ReturnValue(ok=False, error="error during updating of uploaded file to OneDrive subfolder " + folder_id + " " + str(req['status']) + ": " + str(content), restart=False)
...
@celery_app.task(name='tasks.ocr_dummy')
def ocr_dummy(doc, indexno, **kwargs):
    ...
    with flaskapp.app_context():
        with flaskapp.test_request_context(base_url=url_root, path=url):
            flaskapp.preprocess_request()
            # reset_local_variables()
            set_uid(kwargs['user_code'])
            ...
            if doc._is_pdf():  # pylint: disable=protected-access
                return ReturnValue(ok=True, value={'indexno': indexno, 'doc': doc})
            return ReturnValue(ok=True, value={'indexno': indexno, 'doc': pdf_concatenate(doc)})
    ...
    with flaskapp.app_context():
        with flaskapp.test_request_context(base_url=url_root, path=url):
            ...
            if not str(user_info['the_user_id']).startswith('t'):
                ...
                login_user(user_object, remember=False)
            set_request_active(False)
            return ReturnValue(ok=True, value=do_ocr_page(indexno, **kwargs))

@celery_app.task(name='tasks.ocr_finalize')
    ...
    with flaskapp.app_context():
        with flaskapp.test_request_context(base_url=url_root, path=url):
            ...
            if 'message' in kwargs and kwargs['message']:
                ...
                message = word("OCR succeeded")
            try:
                if kwargs.get('pdf', False):
                    try:
                        (target, dafilelist) = do_ocr_finalize(*pargs, **kwargs)
                    except BaseException as e:
                    ...
                    try:
                        assert illegal_variable_name(target.instanceName) is not True
                        ...
                        if dafilelist:
                            assert illegal_variable_name(dafilelist.instanceName) is not True
                    ...
                    if str(user_info.get('the_user_id', None)).startswith('t'):
                        save_user_dict(session_code, user_dict, yaml_filename, secret=secret, encrypt=is_encrypted, steps=steps)
                    else:
                        save_user_dict(session_code, user_dict, yaml_filename, secret=secret, encrypt=is_encrypted, manual_user_id=user_info['theid'], steps=steps)
                    release_lock(session_code, yaml_filename)
                    return ReturnValue(ok=True, value=True)
                return ReturnValue(ok=True, value=message, content=ocr_finalize(*pargs, **kwargs), extra=kwargs.get('extra', None))
...
@celery_app.task(name='tasks.make_png_for_pdf')
def make_png_for_pdf(doc, prefix, resolution, user_code, pdf_to_png, page=None):
    ...
    with flaskapp.app_context():
        with flaskapp.test_request_context(base_url=url_root, path=url):
            ...
            # reset_local_variables()
            set_uid(user_code)
            do_make_png_for_pdf(doc, prefix, resolution, pdf_to_png, page=page)


@celery_app.task(name='tasks.reset_server')
...
@celery_app.task(name='tasks.update_packages')
...
@celery_app.task(name='tasks.email_attachments')
def email_attachments(user_code, email_address, attachment_info, language, subject=None, body=None, html=None, config=None):
    ...
    with flaskapp.app_context():
        with flaskapp.test_request_context(base_url=url_root, path=url):
            ...
            if body is None:
                if len(doc_names) > 1:
                    body = word("Your documents, %s, are attached.") % (comma_and_list(doc_names),)
                else:
                    body = word("Your document, %s, is attached.") % (comma_and_list(doc_names),)
            if html is None:
                html = "<p>" + body + "</p>"
            msg = Message(subject, recipients=[email_address], body=body, html=html)
            success_attach = True
            for attach_info in attachment_info:
                file_info = get_info_from_file_number(attach_info['number'])
    ...
    with flaskapp.app_context():
        with flaskapp.test_request_context(base_url=url_root, path=url):
            ...
            try:
                ...
                error_notification(e, message=error_message, trace=error_trace)
                if 'on_error' not in this_thread.current_info:
            ...
            if interview_status.question.question_type in ["restart", "exit", "exit_logout"]:
                ...
                obtain_lock_patiently(session_code, yaml_filename)
                if str(user_info.get('the_user_id', None)).startswith('t'):
                    reset_user_dict(session_code, yaml_filename, temp_user_id=user_info.get('theid', None))
                else:
                    reset_user_dict(session_code, yaml_filename, user_id=user_info.get('theid', None))
                release_lock(session_code, yaml_filename)
            ...
            if interview_status.question.question_type == "response":
                ...
                if hasattr(interview_status.question, 'all_variables'):
                    ...
                    sys.stdout.write(interview_status.question_text.rstrip().encode('utf8') + "\n")
            ...
            if hasattr(interview_status, 'question_text') and interview_status.question_text:
                ...
                logmessage("background_action: The end result of the background action was the seeking of the variable " + sought_message + ", which resulted in asking this question: " + repr(str(interview_status.question_text).strip()))
                ...
                error_trace = None
                error_message = interview_status.question_text
                variables = list(reversed(list(this_thread.current_variable)))
                error_notification(Exception("The end result of the background action was the seeking of the variable " + sought_message + ", which resulted in asking this question: " + repr(str(interview_status.question_text).strip())))
                if 'on_error' not in this_thread.current_info:
                    return ReturnValue(ok=False, error_type=error_type, error_trace=error_trace, error_message=error_message, variables=variables, extra=extra)
                return process_error(interview, session_code, yaml_filename, secret, user_info, url, url_root, is_encrypted, error_type, error_message, error_trace, variables, extra)
...
@celery_app.task(name='tasks.ocr_google')
