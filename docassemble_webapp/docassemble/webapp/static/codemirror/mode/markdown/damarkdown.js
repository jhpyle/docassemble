...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  var tokenTypes = {
    ...
    mako: "variable-3",
    incodeMarkerexec: "variable-3",
    incodeexec: "variable-3",
    incodeInsideexec: "variable-3",
  ...
  function getType(state) {
    ...
    if (state.linkHref) {
      ...
      if (state.incodeMarkerexec) { styles.push(tokenTypes.incodeMarkerexec); }
      if (state.incodeexec) { styles.push(tokenTypes.incodeexec); }
      if (state.incodeInsideexec) { styles.push(tokenTypes.incodeInsideexec); }
    }
  ...
  function inlineNormal(stream, state) {
    ...
    if (ch === '<' && stream.match(/^%/, false)) {
      //console.log("Got dollar sign");
      state.incodeMarkerexec = true;
      state.incodeexec = true;
      if (modeCfg.highlightFormatting) state.formatting = "code-block";
      return getType(state);
    }

    if (ch === '%' && state.incodeMarkerexec) {
      //console.log("Got start bracket");
      state.incodeMarkerexec = false;
      state.incodeInsideexec = true
      if (modeCfg.highlightFormatting) state.formatting = "code-block";
      return getType(state);
    }

    if (ch === '%' && state.incodeInsideexec && stream.match(/^>/, false)) {
      //console.log("Got end bracket");
      if (modeCfg.highlightFormatting) state.formatting = "code-block";
      var type = getType(state);
      state.incodeInsideexec = false;
      state.incodeMarkerexec = true;
      //state.inline = state.f = linkHref;
      return type;
    }
    ...
    if (ch === '>' && state.incodeMarkerexec) {
      ...
      var type = getType(state);
      state.incodeMarkerexec = false;
      state.incodeexec = false;
      //state.inline = state.f = linkHref;
      return type;
    }

    if (state.incode || state.mako || state.incodeexec){
      return getType(state);
    }
