 ...
 */
(function ($) {
  var toggleHTML = '<div class="toc"><h2>Contents</h2></div>';
  var tocContainerHTML =
    '<div class="card" id="toc-container"><div id="toctitle" class="card-header">%1</div><div id="toc" class="card-body pt-3"><ul>%2</ul></div></div>';

  function createLevelHTML(
    anchorId,
    tocLevel,
    tocSection,
    tocNumber,
    ...
    tocContainer.find("h1").each(function () {
      ...
      h1.nextUntil("h1")
        ...
        .each(function () {
          ...
          if (
            existingAnchor.length > 0 &&
            $(existingAnchor[0]).prop("tagName") == "A" &&
            $(existingAnchor[0]).attr("name")
          ) {
