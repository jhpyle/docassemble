...
if [[ $CONTAINERROLE =~ .*:(all|web|log):.* ]]; then
    ...
    echo -e "# This file is automatically generated by initialize.sh\nWSGIPythonHome ${DA_PYTHON:-/usr/share/docassemble/local}\nTimeout 60\nDefine DAHOSTNAME ${DAHOSTNAME}\nDefine DAPOSTURLROOT ${POSTURLROOT}\nDefine DAWSGIROOT ${WSGIROOT}" > /etc/apache2/conf-available/docassemble.conf
	...
	if [ ! -f /etc/apache2/sites-available/docassemble-ssl.conf ]; then
	    #  || [ "${USELETSENCRYPT:-false}" == "false" ] || [ "${USEHTTPS:-false}" == "false" ]
